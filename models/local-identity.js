  ...
  toJSON() {
    ...
    static fromJSON(obj) {
      ...
      return new LocalIdentity({
        ...
        signature: parsedObj.signature ? base64ToArrayBuffer(parsedObj.signature) : null, // <-- Add this!
        claimedAt: parsedObj.claimedAt, // <-- Add this!
        vdfProof: parsedObj.vdfProof ? {
          ...parsedObj.vdfProof,
          iterations: parsedObj.vdfProof.iterations ? 
            BigInt(parsedObj.vdfProof.iterations.replace('n', '')) : null
        } : null
      });
    }
}
